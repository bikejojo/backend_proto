...
class PagoMutations
{
    public function generatorPayment($root, array $args)
    {
        ...
        $technician=Tecnico::find($paymentData['technicianId']);
        if(!$technician){
            return [
        ...
        $comprobacion=Suscripcion::where('id',$paymentData['subscriptionId'])->where('technicianId',$paymentData['technicianId'])->first();
        if(!$comprobacion){
            return[
    ...
    public function payment($root, array $args)
    {
        ...
        $subscription = Suscripcion::where('id',$payment->subcriptionId);
        if (!$subscription) {
            return [
                'message' => 'No se encontró la suscripción especificada.'
            ];
        }
        $technician=Tecnico::where('id',$subscription->technicianId);
        if(!$technician){
            return [
                'message' => 'No se encontró el tecnico.'
            ];
